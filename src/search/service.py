...
class RecommendationService:
    ...
    def recommend(self, request: RecommendationRequest) -> RecommendationResponse:
        analysis, analysis_fallback = self._analysis(request.query)
        count_range = self._length_range(analysis)
        metadata_filter = self._filter(request.language, count_range)
        ...
        if count_range and len(candidates) < 5:
            dense, sparse = self.pinecone.retrieve(analysis.search_query, self._filter(request.language, None))
            candidates = fuse_chunks(dense, sparse, self.rrf_k)
            relaxed = True
        candidates = candidates[: self.rerank_candidate_count]
        rerank_applied = False
        try:
            scores = self.pinecone.rerank(request.query, candidates)
        ...
        recommendations = self._write(request.query, top_five, request.limit)
